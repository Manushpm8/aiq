...
- The writer produces figures through an on-demand `chart-generation` skill in the new `visualization` skill collection, rendering a sandbox PNG artifact when a sandbox is available and an inline chart spec otherwise; the skill moved out of the always-on writer prompt and out of the `research` collection, so writer configs that relied on `research` for charts must now also assign `visualization`
