...
2. Create a normalized input table with one row per comparable observation. Prefer explicit CSV or JSON records embedded in the Python script. If the source rows are in `/shared/...`, call `read_file` first and embed the returned content in the script, or write a sandbox-local input file under the `sandbox_workdir` from your instructions (per-job; do not hardcode `/workspace` or `/sandbox`). Sandbox code cannot open `/shared/...` directly.
...
3. Call the `execute` tool with a Python command or script that:
   ...
   - uses the `sandbox_workdir` from your instructions for any sandbox-local input or output files (per-job; do not hardcode `/workspace`).
